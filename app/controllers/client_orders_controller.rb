class ClientOrdersController < ApplicationController
    ...
    if model_params["items"].present?
      ...
      if client_order.save
        order_items.each do |product|
          prod = Comee::Core::Product.where(code: product["code"], name: product["name"], description: product["description"], metadata: product["metadata"], product_type_id: product["product_type_id"], unit_id: product["unit_id"]).first_or_create
          Comee::Core::ClientOrderItem.create(client_order_id: client_order.id, product_id: prod.id, quantity: product["quantity"], price: product["price"])
        end
  ...
  def model_params
    params.require(:payload).permit(:order_number, :order_date, :delivery_date, :client_id, :status, :delivery_address, :invoice_address, :remark, terms: {}, items: {})
  end
end
